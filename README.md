...
- **颜色、间距、圆角、阴影只在 `packages/ui/styles/tokens.css` 中定义**，
  组件内通过 `var()` 引用，不写死具体数值；改一处，学生端与管理端同时生效。
...
整体风格：简洁、克制，把注意力让给商品本身。几条具体约定：

- **商品不是「卡片」**：列表项由「照片 + 价格 + 标题 + 卖家」组成，
  没有边框、底色和阴影。整屏二十个一模一样的圆角盒子最容易暴露模板感。
- **价格紧贴照片**，并且是页面上唯一使用强调色的元素。照片高度固定，
  价格因此能横向连成一条线，一眼就能比较贵贱。
- **价格用衬线数字**（`--font-numeric`）。中文电商的价格清一色是粗黑体，
  衬线读起来更像价签而不是促销牌，也更贴合二手交易该有的语气。
- **分类是导航，不是内容**：用图标宫格呈现，不给每个导航项套卡片和箭头。
- **容器按层级区分**：只有「卖家信息」这类独立实体才用带边框的容器，
  正文区块用留白和分隔线分组。
- **不用「A · B · C」拼元信息**，改用间距与标签分组。
